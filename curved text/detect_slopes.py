...
def find_character_boxes(image):
    ...
    for i, stat in enumerate(stats):
        ...
        elif w*h > 0.6*imgh*imgw or w*h < 30 or ( w*h>7000 and w/h > 3 ):#
            pass
...
if __name__ == "__main__":
    ...
    if args["visuals"]:
        ...
        cv2.imshow("curve",~skeleton.astype(np.uint8))
        ...
        cv2.imwrite("output-images/"+ output_image_name +"_dilate.jpg",dilation)
        cv2.imwrite("output-images/"+ output_image_name +"_erode.jpg",erosion)
        cv2.imwrite("output-images/"+ output_image_name +"_skel.jpg",skeleton)
        cv2.imwrite("output-images/"+ output_image_name +"_curve.jpg",~skeleton)
        cv2.imwrite("output-images/"+ output_image_name +"_thin.jpg",thinning.astype(np.uint8)*255)
        cv2.imwrite("output-images/"+ output_image_name +"_comps.jpg",vr_image)
