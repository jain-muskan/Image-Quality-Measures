...
#to check if there are any boxes within the big box
...
def find_small_white_comps(image):
    ...
    small_rect_stats = []
    sorted_stats = stats[np.argsort(stats[:, 1])]
    ...
    for i, stat in enumerate(sorted_stats):
        ...
        #detects huge boxes based on its area and comparatively small boxes if there are any other pixels in it 
            ...
            if w<= 3 and h<=3:
                count_small_rects+=1
                small_rect_stats.append(stat[0:2])
                cv2.rectangle(image1,(x,y),(x+w,y+h),(0,0,255),1)

    #print(small_rect_stats)

    print("Total rects :",count_total_rects,",Small rects :",count_small_rects)
    wsf = 0 if count_total_rects==0 else count_small_rects/count_total_rects
    print("WSF :",wsf)

    return image1 , small_rect_stats

def check_box_count(x1,y1,x2,y2,small_rect_stats):
    count = 0
    for x,y in small_rect_stats:
        if y >  y2 :
            break
        if y >= y1 and x >= x1 and x<=x2 and y<=y2:
            count += 1
    return count

def find_speckel_factor(binary , output_image_name):
    drawn_image , small_rect_stats = find_small_white_comps((~binary).astype(np.uint8))
    ...
    for i, stat in enumerate(sorted_stats):
        ...
        #detects huge boxes based on its area and comparatively small boxes if there are any other pixels in it 
        elif w*h >3500 or ( w*h > 1700 and check_box_not_empty(sorted_stats[i:],x,y,x+w,y+h) ):
            pass

        else:
            if check_box_count(x,y,x+w,y+h,small_rect_stats) > 0:
